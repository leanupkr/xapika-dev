...
| **P0** | VISION IT 공식 웹사이트 URL 확정 (visionit.ai vs visionit.kr) | ⚠️ 불일치 |
...
**현재 임시값**
- 카피: "Awaiting CEO message — content arriving from Harika" (영) / "CEO 메시지 도착 대기 · 하리카 제공 예정" (한)
- 이름: "CEO Name · Founder & CEO" (임시 라벨)
- 초상: 텍스트 placeholder ("Portrait · 추후 제공")

**필요 형식**
- 본문: 영문·한글 각 200~600 단어 (장문 단락 2-4개 권장)
...
**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `about.ceo.*` 키
- `public/about/ceo-portrait.{jpg|png}` (신규 추가)
...
**노출 위치**
- `/locations` — Locations 페이지의 9개 오피스 카드
...
| 오피스 | 주소 (현지어 + 영문) | 대표 전화 | 대표 이메일 |
|---|---|---|---|
| Warsaw HQ (폴란드 본사) | 필요 | 필요 | `info@xapika.pl` 외 추가 있을 경우 |
| Warsaw Office (폴란드 오피스) | 필요 | 필요 | 필요 |
| Kyiv (우크라이나) | 필요 | 필요 | 필요 |
| Seoul (한국) | **확인됨** ⬆ | 필요 | 필요 |
| São Paulo (브라질) | 필요 | 필요 | 필요 |
| Virginia (미국) | 필요 | 필요 | 필요 |
| Istanbul (튀르키예) | 필요 | 필요 | 필요 |
| Tashkent (우즈베키스탄) | 필요 (2026.04 오픈) | 필요 | 필요 |
| Cairo (이집트) | 필요 | 필요 | 필요 |

- 구성안 §1.6 명시: "**주소, 건물 사진은 구글 지도 위치로 대체, 연락처 전달 예정**" → 주소만 받으면 구글 지도 핀 자동 연결됩니다.

**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `locationsPage.offices[]` 배열 안 각 객체에 `address`, `phone`, `email` 필드 추가
...
**노출 위치**
- `/solutions/digital-asset-management` — `Go to VISION IT` 외부 링크 버튼 (Hero 영역 + CTA 영역)

**불일치 내용**
- VISION IT 우크라이나어판 브로셔 표지: **`www.visionit.ai`**
- 현재 사이트 i18n 값: **`https://visionit.kr`**
...
**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `solutionsDetail.digitalAssetManagement.cta.visitVisionIt` 값
...
- 사이트 카피: "Coming May 2026" / "운영 개시 2026.05.31 · 타슈켄트"
- 5월 31일 일자는 LEANUP의 일정표 마지막 납품일과 동일하여 임시 설정 — **실제 운영 개시일이 다르면 알려 주세요.**
...
**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `portfoliosDetail.uzbekistan.*` 및 `portfolios.uzbekistan.*`
- `locationsPage.offices` 배열 중 `tashkent`의 `since` 필드
...
**현재 임시값**
- "Organization Chart · diagram arriving from Harika" 텍스트 placeholder
...
- 데이터: `src/messages/en.json` + `ko.json` 의 `about.organization.*`
...
- **WhatWeDo — 수행 업무 목록** (현재 5 items / 3 items, 영문 + 한글)
- **KeyStats — 4개 수치 또는 정성 메트릭** (현재 구성안 수치만 노출)
...
**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `solutionsDetail.{lightMaintenance|heavyMaintenance|supplyChain|digitalAssetManagement|commercialServices}.*`
...
|---|---|---|
| **VISION IT** | ✅ 로고 적용됨 | — |
| **Intel** | ✅ Intel Gold Alliance 로고 적용됨 | — |
...
**현재 상태**
- 우즈베키스탄 운영은 **2026.05 개시 예정**이라 현재 사진·영상 없음
...
- `src/messages/en.json` + `ko.json` 의 `portfoliosDetail.uzbekistan.gallery.*`
...
**필요 형식**
- 정식 본문 (영문 + 한글), Markdown 또는 워드 파일
- EU GDPR + 폴란드 개인정보보호법 + 한국 개인정보보호법 부합 권장
- 본문 구조 권장: 수집 항목 / 보관 기간 / 처리 목적 / 정보주체의 권리 / 문의처

**받은 후 교체 위치**
- `src/messages/en.json` + `ko.json` 의 `privacyPage.*`, `termsPage.*`
- 본문이 길면 `src/content/privacy.{en,ko}.mdx` 같은 MDX 파일로 분리 가능 (LEANUP 작업)
...
- HRCS2 Depot Photos: 38장 (현장)
- Warsaw Tram Photos: 21장 (현장)
- DropMeFiles_K7A0i: 281개 raw 자산
...
**파일명 권장**
- 카피 (텍스트): `[페이지명]_[영문/한글].docx` 또는 Markdown
...
i18n 파일 두 개를 직접 수정하셔도 됩니다. 자세한 절차는 `OPERATIONS.md` 참고.

```
src/messages/en.json    # 영문 본문
src/messages/ko.json    # 한국어 본문
```

**원칙**
- 두 파일을 **항상 같이** 수정해 주세요 (한쪽만 바뀌면 다른 언어에 옛 문구가 그대로 나옵니다)
...
| Locations 오피스 | `locationsPage.offices[]` |
